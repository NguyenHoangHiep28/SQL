...
CREATE TABLE NhanVien(
	...
	SoCMND char(9) CHECK (Len(SoCMND) = 9 AND SoCMND NOT LIKE '%[^0-9]%'),
...
--5
GO
SELECT MaNV, TenNV, Sum(SoTien) FROM LuongDA   GROUP BY MaNV 
--6
...
GROUP BY MaPB
--16
SELECT MaDA, Count(MaNV) AS SoNV FROM LuongDA GROUP BY MaDA HAVING COUNT(MaNV) >= 2
--17
...
--19
SELECT TenNV, NgaySinh, SoCMND, GioiTinh, DiaChi, NgayVaoLam, MaPB FROM
NhanVien WHERE MaNV NOT IN (SELECT MaNV FROM LuongDA)
--20
